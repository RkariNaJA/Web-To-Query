...
// Lint is relaxed for this file ONLY, so it can stay byte-comparable with its
// vanilla twin and keep diffing cleanly against it:
//   no-unused-vars   — errOnly / tbodyRows / joinOrDash are dead in the vanilla
//                      source too. Verified harmless: each popup ships an empty
//                      <tbody> and fills it client-side via buildTable().
//   no-useless-escape — the `<\/script>` escapes are deliberate, so these
//                      templates stay safe if ever inlined into an HTML page.
//
// Ported verbatim from the vanilla app's js/reports.js.
// Each report renders a self-contained HTML document into a new window: it
// carries its own styles, its own theme-remap script, and (for BotPO) its own
...
// ── Error Summarize ────────────────────────────────────────────────
export function showErrorSummary(rows) {
  if (!rows.length) return;
  ...
  <div class="wrap">
    ...
    <div style="display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;padding:16px 0 20px;">
      ...
      <div><div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#6b7494;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:4px;">Size</div>
        <select id="f-size" onchange="applyFilter()" style="background:#1a1e28;border:1px solid #3a4258;color:#e2e6f0;font-family:'IBM Plex Mono',monospace;font-size:11px;padding:5px 8px;border-radius:6px;min-width:110px;"><option value="">All</option>${[...new Set(rows.map(r => getVal(r, 'INVENTSIZEID')).filter(v => v && v !== '—'))].sort().map(v => `<option value="${v}">${v}</option>`).join('')}</select></div>
      <div><div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#6b7494;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:4px;">Color</div>
        <select id="f-color" onchange="applyFilter()" style="background:#1a1e28;border:1px solid #3a4258;color:#e2e6f0;font-family:'IBM Plex Mono',monospace;font-size:11px;padding:5px 8px;border-radius:6px;min-width:110px;"><option value="">All</option>${[...new Set(rows.map(r => getVal(r, 'INVENTCOLORID')).filter(v => v && v !== '—'))].sort().map(v => `<option value="${v}">${v}</option>`).join('')}</select></div>
      <div><div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#6b7494;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:4px;">Season</div>
        <select id="f-season" onchange="applyFilter()" style="background:#1a1e28;border:1px solid #3a4258;color:#e2e6f0;font-family:'IBM Plex Mono',monospace;font-size:11px;padding:5px 8px;border-radius:6px;min-width:110px;"><option value="">All</option>${[...new Set(rows.map(r => getVal(r, 'INVENTSEASONID')).filter(v => v && v !== '—'))].sort().map(v => `<option value="${v}">${v}</option>`).join('')}</select></div>
...
// ── Unit Summarize ─────────────────────────────────────────────────
export function showUnitSummary(rows) {
  if (!rows.length) return;
...
export function showCheckSummary(rows) {
  if (!rows.length) return;

  const uniqVals = key => [...new Set(rows.map(r => getVal(r, key)).filter(v => v && v !== '—'))].sort();
