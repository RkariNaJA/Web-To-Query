...
function buildFullErrorHtml(rows) {
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <div style="overflow-x:auto;border:1px solid #252a38;border-radius:8px;">
        <table style="width:100%;border-collapse:collapse;font-family:'IBM Plex Mono',monospace;font-size:12px;">
          <thead><tr style="background:#1a1e28;">${['LINE','ITEM ID','SIZE','COLOR','SEASON','QTY','PRICE','STATUS'].map(h=>`<th style="padding:10px 14px;text-align:left;font-size:10px;letter-spacing:0.07em;color:#6b7494;text-transform:uppercase;border-bottom:1px solid #252a38;white-space:nowrap;">${h}</th>`).join('')}</tr></thead>
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Full Error Report</title>
    ...
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:28px;padding-bottom:20px;border-bottom:1px solid #252a38;">
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:#6b7494;margin-top:3px;">${rows.length} error lines across ${Object.keys(poGroups).length} PO${Object.keys(poGroups).length!==1?'s':''}</div></div>
...
function buildErrorSummaryHtml(rows) {
  ...
  const tbodyRows = Object.entries(poGroups).sort(([,a],[,b])=>{
    ...
    const groupRow = `<tr><td colspan="7" style="padding:10px 14px;background:#13161d;border-top:2px solid #3a4258;border-bottom:1px solid #252a38;">
      <span style="font-family:'IBM Plex Mono',monospace;font-size:12px;font-weight:600;color:#4f9cf9;">${poid}</span>
      <span style="margin-left:8px;font-size:11px;color:#6b7494;">· ${execId}</span>
      ...
      <span style="margin-left:6px;font-size:10px;padding:2px 7px;border-radius:3px;background:rgba(79,156,249,0.1);color:#4f9cf9;border:1px solid rgba(79,156,249,0.25);">${Object.keys(itemGroups).length} ITEM${Object.keys(itemGroups).length!==1?'S':''}</span>
    ...
    const itemRows = Object.entries(itemGroups).map(([item,irows])=>{
      ...
      return `<tr style="${isDup?'background:rgba(251,146,60,0.04);':''}">
        ...
        <td style="color:#6b7494;font-size:11px;">${irows.map(r=>getVal(r,'LINENUMBER')??'—').join(', ')}</td>
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Error Summary</title>
    ...
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:28px;padding-bottom:20px;border-bottom:1px solid #252a38;">
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:#6b7494;margin-top:3px;">${totalPOs} PO${totalPOs!==1?'s':''} · ${totalItems} unique item${totalItems!==1?'s':''} with errors</div></div>
      <button onclick="window.print()" style="margin-left:auto;padding:8px 16px;background:#1a1e28;border:1px solid #3a4258;color:#6b7494;font-family:'IBM Plex Mono',monospace;font-size:11px;border-radius:6px;cursor:pointer;">⎙ Print</button>
    </div>
    <div style="overflow-x:auto;border:1px solid #252a38;border-radius:8px;">
      <table style="width:100%;border-collapse:collapse;font-family:'IBM Plex Mono',monospace;font-size:12px;">
        <thead><tr style="background:#1a1e28;">${[th('ITEM ID'),th('ERROR LINES'),th('LINE NO.'),th('SIZES'),th('COLORS'),th('SEASONS'),th('STATUS')].join('')}</tr></thead>
